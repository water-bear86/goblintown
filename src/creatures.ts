...
const DEFAULT_LOW_COST_MODEL = process.env.GOBLINTOWN_MODEL_GOBLIN ?? "gpt-5-mini";
...
export function makeGoblin(personality: Personality = "nerdy"): Creature {
  return {
    ...
    systemPrompt:
      ...
      `No preamble, no apology, no meta-commentary. Default policy is low-cost, high-throughput generation. ` +
      `Be specific, dense, and useful.` +
...
export function makeGremlin(personality: Personality = "feral"): Creature {
  return {
    kind: "gremlin",
    modelSlot: "gremlin",
    model: process.env.GOBLINTOWN_MODEL_GREMLIN ?? DEFAULT_LOW_COST_MODEL,
    temperature: 1.1,
    personality,
    systemPrompt:
      ...
      `Favor a lower-cost adversarial pass by staying compact and evidence-first.` +
...
export function makeRaccoon(personality: Personality = "stoic"): Creature {
  return {
    ...
    systemPrompt:
      ...
      `Your job is scavenging and research. You receive a task, context dump (file contents, logs, prior loot), ` +
      `and DAG context. Return only the facts that matter for the task. No speculation, no rephrasing. ` +
      `If a fact is missing, say so explicitly with "MISSING: <what>". ` +
      `When needed, provide only public source-backed web references that an assistant can verify. ` +
      `Use assistant-callable DAG/context reads as the first source of truth, then web references for external validation.` +
      personalityTag(personality),
  };
}
...
export function makeTroll(personality: Personality = "cynical"): Creature {
  return {
    ...
    systemPrompt:
      ...
      `Your job is adversarial verification. You receive (a) the original task and (b) a candidate output from a Goblin. ` +
      `Your default is to reject. Only pass an output that is materially correct, complete, and on-task. ` +
      `If enabled, use verifier tools before scoring to prove JSON/regex/http assumptions; otherwise call out evidence limits clearly. ` +
...
export function makeOgre(personality: Personality = "stoic"): Creature {
  return {
    ...
    systemPrompt:
      ...
      `You are the heavyweight: large context, slow, and expensive. ` +
      `Run only as a premium fallback when the Goblin pack and specialist recovery fail. ` +
...
export function makePigeon(personality: Personality = "chipper"): Creature {
  return {
    ...
    systemPrompt:
      ...
      `Use embedding-friendly phrasing and DAG curation tags for downstream routing. ` +
...
export function makeScribe(personality: Personality = "stoic"): Creature {
  return {
    kind: "pigeon",
    modelSlot: "scribe",
    model: process.env.GOBLINTOWN_MODEL_SCRIBE ?? "gpt-5-mini",
    temperature: 0.2,
    personality,
    systemPrompt:
